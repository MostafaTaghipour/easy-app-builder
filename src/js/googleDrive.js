...
const GoogleDrive = require('./js/helpers/googleDrive');
...
const {pathToCredentialsJsonFile} = configs.getPrefs().googleDriveUpload;

const drive = new GoogleDrive(
  helpers.replacePlaceHolders(pathToCredentialsJsonFile),
);
...
title.innerHTML = `Upload ${_fileName} to GooglePlay`;
...
async function startUploading() {
  ...
  try {
    ...
    setStep('upload');
    const id = await drive.uploadFileAsync(zipPath || _filePath);
    await drive.shareFileAsync(id);
    const {webViewLink} = await drive.getFileInfoAsync(id);
