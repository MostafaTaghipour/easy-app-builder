...
const Diawi = function (opts) {
  ...
  this.fileSize = fs.lstatSync(this.path).size;
...
Diawi.prototype.execute = function () {
  const r = request.post(
    {url: UPLOAD_URL, formData: this.formData},
  ...
  this.q = setInterval(function () {
    var dispatched = r.req.connection._bytesDispatched;
    let percent = Math.floor((dispatched * 100) / size);
    if (DEBUG) console.dir('Uploaded: ' + percent + '%');
    self.emit('progress', percent);
  }, 250);
};

Diawi.prototype.onUploadComplete = function (err, response, body) {
  if (this.q) clearInterval(this.q);
      ...
      request.get(
        ...
        function (err, response, body) {
          ...
          try {
            ...
            switch (json.status) {
              case 2000:
                if (json.link && json.qrcode) {
                  this.emit('complete', json.link, json.qrcode);
                } else {
                  this.emit(
                    'error',
                    new Error(
                      'Failed to get link and qr from success response',
                    ),
                  );
                }
              ...
              default:
                this.emit(
                  'error',
                  new Error('Error in status response - ' + json.message),
                );
                return;
            }
          } catch (err) {
